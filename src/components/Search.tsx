...
export default function Search({ nodes, colors, onPick, onClose }: Props) {
  ...
  return (
    <div className={styles.backdrop} onPointerDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className={styles.box} role="dialog" aria-label="Search">
            ...
            {results.map((r, i) => (
              <li key={r.n.id} role="option" aria-selected={i === active}>
                ...
                >
                  ...
                  <span className={styles.kind}>{r.n.kind === "main" ? "main" : "sub"}</span>
                </button>
              </li>
            ))}
          </ul>
        ) : (
          <div className={styles.empty}>{nodes.length === 0 ? "No modules yet." : "No matching modules."}</div>
        )}
      </div>
    </div>
  );
